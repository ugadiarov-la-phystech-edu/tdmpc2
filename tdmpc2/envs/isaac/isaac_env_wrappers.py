...
def make_env(cfg):
	...
	folder = os.path.dirname(inspect.getsourcefile(make_env))
	isaac_env_cfg = yaml.safe_load(Path(os.path.join(folder, 'config/generalization_sort_push/IsaacPandaPushConfig.yaml')).read_text())
	isaac_env_cfg['env']['numObjects'] = num_objects
	isaac_env_cfg['env']['numColors'] = num_objects
	isaac_env_cfg['env']['cameraRes'] = cfg['obs_size']
	isaac_env_cfg['env']['episodeLength'] = cfg['time_limit']
	env = IsaacPandaPush(
		cfg=isaac_env_cfg,
		rl_device=f"cpu",
		sim_device=f"cpu",
	...
	isaac_wrapper_cfg = yaml.safe_load(Path(os.path.join(folder, 'config/generalization_sort_push/Config.yaml')).read_text())
	isaac_wrapper_cfg['Reward']['GT']['use_out_of_field_reward'] = use_out_of_field_reward
	env = IsaacPandaPushGoalSB3Wrapper(
		...
		reward_cfg=isaac_wrapper_cfg['Reward']['GT'],
		smorl=(isaac_wrapper_cfg['Model']['method'] == 'SMORL'),
	)
...
def load_pretrained_rep_model(*args, **kwargs):
	return None


def load_latent_classifier(*args, **kwargs):
	return None


def get_dlp_rep(*args, **kwargs):
	return None


def extract_dlp_image(*args, **kwargs):
	return None
...
class IsaacPandaPushVectorized:
	def __init__(self, cfg, isaac_panda_push_goal_wrapper):
		self.cfg = cfg
		self.env = isaac_panda_push_goal_wrapper
		self._last_frame = None
		observation_space = self.env.observation_space['achieved_goal']
		self.observation_space = gymnasium.spaces.Box(low=0, high=255, shape=(*observation_space.shape[2:], observation_space.shape[1]), dtype=np.uint8)
		action_space = self.env.action_space
		self.action_space = gymnasium.spaces.Box(low=action_space.low, high=action_space.high, shape=action_space.shape, dtype=action_space.dtype)
		self.max_episode_steps = self.cfg.get('episode_length')
	...
	def render(self, env_ids=None):
		...
		return self._last_frame[env_ids].copy()

	@staticmethod
	def _get_image_from_obs_dict(obs_dict):
		return obs_dict['achieved_goal'][0][0].transpose(1, 2, 0)

	def step(self, action, env_ids=None):
		if env_ids is not None and len(env_ids) != self.cfg.num_envs:
			raise ValueError('Does not support partial step')

		observation, reward, done, info = self.env.step(action)
		self._last_frame = self._get_image_from_obs_dict(observation)

		if not done.all().items() and done.any().item():
			raise ValueError(f'Does not support partial resets, but got: {done}')

		return self.render(), reward, torch.zeros_like(done), done, info

	def reset(self, env_ids=None):
		if env_ids is not None and len(env_ids) != self.cfg.num_envs:
			raise ValueError('Does not support partial resets')

		observation = self.env.reset()
		self._last_frame = self._get_image_from_obs_dict(observation)
		return torch.from_numpy(self.render())


class IsaacPandaPushGoalSB3Wrapper(GoalEnv):
	def __init__(self, env, obs_mode, n_views, latent_rep_model, latent_classifier, reward_cfg, smorl=False, **kwargs):
		...
		self.use_eff_reward = reward_cfg.get("use_eff_reward", False)
		self.use_out_of_field_reward = reward_cfg.get("use_out_of_field_reward", False)
	...
	def step(self, action):
		...
		prev_eff_pos = self.env.obs_dict['obs'][:, :1].cpu().numpy()
		prev_cube_pos = self.env.obs_dict['obs'][:, 1:].cpu().numpy()
		...
		vec_info = {
			...
			"eff_pos": obs_dict['obs'][:, :1].cpu().numpy(),
			"current_cube_pos": obs_dict['obs'][:, 1:].cpu().numpy(),
			"prev_eff_pos": prev_eff_pos,
	...
	def compute_reward(self, achieved_goal, desired_goal, info={}):
		...
		if self.push_t:
			...
			prev_a_goal = info['prev_cube_pos'][..., :2] / table_diag_len
			prev_dist = np.linalg.norm(prev_a_goal - d_goal, axis=-1)

			if self.use_out_of_field_reward:
				# Negative reward when cubes are out of the table
				xy_scale = np.array(self.table_dims[:2], dtype=np.float32) / 2
				normalized_position = info['position'] / xy_scale
				reward = -np.sum(
					np.any(np.abs(normalized_position) >= 1, axis=-1) | (
								np.abs(info['current_cube_pos'][..., 2] - 1.0425) >= 0.04),
					axis=-1
				).astype(np.float32)
			else:
				reward = 0
			...
			# calculate reward
			if self.random_obj_num:
				assert f'random_obj_num is not supported in the setting with diff reward'
				reward += -np.sum(dist, axis=-1) / cur_num_obj
			else:
				reward += np.sum(prev_dist - dist, axis=-1)

			if self.use_eff_reward and 'eff_pos' in info and 'current_cube_pos' in info:
				current_cube_pos = info['current_cube_pos'] / table_diag_len
				eff_pos = info['eff_pos'] / table_diag_len
				eff_to_cubes = np.linalg.norm(current_cube_pos - eff_pos, axis=-1)

				prev_cube_pos = info['prev_cube_pos'] / table_diag_len
				pref_eff_pos = info['prev_eff_pos'] / table_diag_len
				pref_eff_to_cubes = np.linalg.norm(prev_cube_pos - pref_eff_pos, axis=-1)

				eff_reward = np.sum(np.cbrt(pref_eff_to_cubes) * prev_dist - np.cbrt(eff_to_cubes) * dist, axis=-1)


		# rescale reward
		reward = reward * self.reward_scale + eff_reward * self.reward_scale / 4
