...
from ..wrappers.pixels import Pixels
...
class CwTargetEnvGymnasium(gymnasium.Env):
	...
	def __init__(self, cfg):
		...
		observation_space = (cfg.obs_image_size, cfg.obs_image_size, 3)
		self.observation_space = gymnasium.spaces.Box(0, 255, observation_space, dtype=np.uint8, seed=seed)
...
class SelectObsKeyWrapper(gym.Wrapper):
	...
	def step(self, action):
		self._last_observation, reward, done, info = super().step(action)
		return self.observation(), reward, done, info

	def observation(self):
		return self._last_observation[self.obs_key].copy()
