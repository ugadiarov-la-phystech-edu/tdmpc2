...
class ManiSkillWrapper(gym.Wrapper):
	def __init__(self, env, cfg, frame_skip=1):
		...
		self.env = env
		self.cfg = cfg
		self.observation_space = gym.spaces.Box(low=0, high=255, shape=(self.cfg.obs_image_size, self.cfg.obs_image_size, 3),
												dtype=np.uint8)
	...
	def _process_observation(self, observation):
		...
		self.last_observation = np.array(
			Image.fromarray(self.last_source_observation).resize((self.cfg.obs_image_size, self.cfg.obs_image_size))
		)
