...
def build_ocr_model(cfg):
	if cfg.ocr_model == 'ksavi':
		...
		akornsavi_config = types.SimpleNamespace(**cfg.ocr_config)
		n_patches = (cfg.obs_image_size // akornsavi_config.psize) ** 2
		encoder = AKOrN(
			akornsavi_config.N,
			ch=akornsavi_config.ch,
			L=akornsavi_config.L,
			T=akornsavi_config.T,
			J=akornsavi_config.J,
			use_omega=akornsavi_config.use_omega,
			global_omg=akornsavi_config.global_omg,
			c_norm=akornsavi_config.c_norm,
			psize=akornsavi_config.psize,
			imsize=cfg.obs_image_size,
			autorescale=akornsavi_config.autorescale,
			maxpool=akornsavi_config.maxpool,
			project=akornsavi_config.project,
			heads=akornsavi_config.heads,
			use_ro_x=akornsavi_config.use_ro_x,
			no_ro=akornsavi_config.no_ro,
			gta=akornsavi_config.gta,
		)
		...
		features_projector = MLP(
			inp_dim=akornsavi_config.ch,
			outp_dim=akornsavi_config.slot_size,
			hidden_dims=[2 * 256],
			initial_layer_norm=True, )

		initializer = Learned(num_slots=akornsavi_config.num_slot, slot_dim=akornsavi_config.slot_size)

		slot_attention = Corrector(
			num_slots=akornsavi_config.num_slot,
			slot_dim=akornsavi_config.slot_size,
			feature_dim=akornsavi_config.slot_size,
			num_iterations=1,
			num_initial_iterations=3,
			hidden_dim=4 * akornsavi_config.slot_size,
		)

		decoder = MLPDecoder(
			inp_dim=akornsavi_config.slot_size,
			outp_dim=akornsavi_config.ch,
			hidden_dims=[512, 512, 512],
			n_patches=n_patches)
		predictor = TransformerPredictor(slot_dim=akornsavi_config.slot_size, action_dim=-1,)
		ocr_model = AkornSAVi(encoder, features_projector, initializer, slot_attention, decoder, predictor,
							 is_encoder_frozen=True)
		weights = torch.load(cfg.ocr_checkpoint_path, weights_only=True)['model']
		...
		raise ValueError(f'oc_model {cfg.oc_model} not supported')
...
class SlotExtractor:
	...
	def __call__(self, images: torch.Tensor, prev_slots: torch.Tensor, to_numpy=False):
		...
		if prev_slots is not None and len(prev_slots.shape) == 2:
			batch_prev_slots = prev_slots.unsqueeze(0)
		else:
			batch_prev_slots = prev_slots

		batch_images = batch_images.to(self._device) / 255.
		if batch_prev_slots is not None:
			batch_prev_slots = batch_prev_slots.to(self._device)

		slots = self._model.get_slots(batch_images, batch_prev_slots, sequence_start=batch_prev_slots is None).detach().cpu()
		if len(images.shape) == 3:
			slots = slots[0]
