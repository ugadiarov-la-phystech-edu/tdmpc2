...
class AkornSAVi(nn.Module):
	...
	def get_slots(self, images: torch.Tensor, prev_slots: torch.Tensor = None, sequence_start: bool = False) -> torch.Tensor:
		return self.forward(images.unsqueeze(1), actions=torch.empty(0, 1), prior_slots=prev_slots, step_offset=int(not sequence_start))['slots_sequence'][:, 0]
